...
public class Game {
    ...
    private static InputController inputController = new InputController(control);
    public static MainLoop gameLoop = new MainLoop();
    ...
    public static void createNPCs() throws Exception{
        // Adding NPCs
        world.map.activeRoom.addNPC(0, 0, (int)(world.map.getActiveRoom().size()/2 - world.player.width/2), (int)(world.map.getActiveRoom().size()/2 - world.player.height/2));
    }

    public static void createTriggerMaps() throws Exception{
        int borderOffset = MapGenerator.TILE_SIZE * MapGenerator.TILE_SCALE_FACTOR * 3;
        TriggerMap triggerMap = new TriggerMap(world.player, world.map.getActiveRoom().getTileMap());
        world.setTriggerMap(triggerMap);
        
        DialogHud doorHud = (DialogHud)world.huds.get("doorClosed");
        // Door dialog
        ...
        PlayerHud playerHud = (PlayerHud)world.huds.get("hud");
        triggerMap.addTrigger(1, new Trigger() {

            @Override
            public void onTriggered() {
                Room up = world.map.getAdjacentRoom(Directions.UP);
                loadRoom(new Coords((int)world.player.px, borderOffset), world, up, doorHud, playerHud, bossDoorClosed, doorClosed);
            }
        ...
        triggerMap.addTrigger(2, new Trigger() {
            ...
            public void onTriggered() {
                ...
                loadRoom(new Coords(world.map.getActiveRoom().size() - borderOffset, (int)world.player.py), world, left, doorHud, playerHud, bossDoorClosed, doorClosed);
            }
        ...
        triggerMap.addTrigger(3, new Trigger() {
            ...
            public void onTriggered() {
                ...
                loadRoom(new Coords((int)world.player.px, world.map.getActiveRoom().size() - borderOffset), world, down, doorHud, playerHud, bossDoorClosed, doorClosed);
            }
        ...
        triggerMap.addTrigger(4, new Trigger() {
            ...
            public void onTriggered() {
                ...
                loadRoom(new Coords(borderOffset, (int)world.player.py),world, right, doorHud, playerHud, bossDoorClosed, doorClosed);
            }
    ...
    public static void createHUDs() throws Exception{
        /** DOOR HUD */
        DialogHud doorHud = new DialogHud(display, display.getWidth()/2 - (int)(712*1.5/2),0, display.getWidth(),display.getHeight()/5);
        ...
        PlayerHud playerHud = new PlayerHud(display, world.player, world.shop);
        ...
        Menu menu = new Menu(display, playerHud,0, 0, display.getWidth(), display.getHeight());
        world.addHud("menu", menu);

        
        GameOver gameOver = new GameOver(display, 0, 0, display.getWidth(), display.getHeight());
        gameOver.setIsShown(false);
        gameOver.setInteractable(false);
        world.addHud("gameOver", gameOver);

        EndMenu endMenu = new EndMenu(display, 0, 0, display.getWidth(), display.getHeight());
        endMenu.setIsShown(false);
        endMenu.setInteractable(false);
        world.addHud("endMenu", endMenu);

        DialogHud npcHud = new DialogHud(display, display.getWidth()/2 - (int)(712*1.5/2),0, display.getWidth(),display.getHeight()/5);
    ...
    public static void createPlayer() throws Exception{
        world.setPlayer(0, 0,(int)(world.map.getActiveRoom().size()/2 - 32/2),20, 100, 10);
        InputController.setPlayer(world.player);
    }

    public static void createWorld() throws Exception{
        world = new World();
        Room.count = 0;
        ...
        world.setMap(MapGenerator.GenerateMap(world, startRoom, endRoom, 5, 5, 5));
        display = new Display(world, world.map);
        ...
        MapGenerator.populateMap(world.map, 0, 3, new ArrayList<Integer>(
        ...
        gameLoop.gamePhysics = new GamePhysics();
        gameLoop.gamePhysics.display = display;
        gameLoop.gamePhysics.physicsEngine = physicsEngine;
    }

    public static void createDialogs() throws Exception{

        // Guardian dialog
        Dialog npc1 = new Dialog();
        npc1.addLine(new String[] {
            "Bienvenue aventurier ! ",
            "Appuiez sur «$» pour parler au gardien afin qu'il vous donne votre première quête."
        });
        npc1.addLine(new String[] {
    ...
    public static void createLoop() throws Exception{
        gameLoop.cKey = inputController;
        gameLoop.start();
    }
    ...
    public static void startGame() throws Exception{
        // System.out.println("startGameCalled");
        createWorld();
        createPlayer();
        createShop();
        createHUDs();
        ...
        createNPCs();
        createDialogs();
        // world.huds.get("gameOver").setIsShown(true);
    }

    
    private static void printKeyAdapters(Component component) {
        if (component instanceof Container) {
            Container container = (Container) component;
            for (Component child : container.getComponents()) {
                printKeyAdapters(child); // Recursively check child components
            }
        }
        
        // Check if the component has any KeyListeners
        if (component instanceof JComponent) {
            JComponent jComponent = (JComponent) component;
            for (KeyListener listener : jComponent.getKeyListeners()) {
                System.out.println("- " + listener.getClass().getName() + " attached to " + component.getClass().getName());
            }
        }
    }
    ...
    */
    public static void main(String[] args) throws Exception {
        startGame();
        // Print all KeyListeners
        printKeyAdapters(Display.frame);
        createLoop();
    }
